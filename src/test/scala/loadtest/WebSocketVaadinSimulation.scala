...
class WebSocketVaadinSimulation extends Simulation {
  ...
  .group("Create Process") {
    ...
    .exec(
      ...
      .check(wsAwait.within(5).until(1).regex("joined").find(0).exists)
    )
    .pause(5 seconds)
    .exec(
      ws("Suggest answer 3")
      ...
      .check(wsAwait.within(5).until(1).regex("wrong").find(0).exists)
    )
    .pause(1 seconds)
  ...
  setUp(scn.inject(rampUsers(1) over (3 seconds))).protocols(httpProtocol)
